(function($){
			$.fn.extend({
				tyslide:function(opts){
					...
					var prams=$.extend({},defaults,opts);//用opts里 去覆盖defaults里面的属性，最后放到空对象{}里面，prams就是合并之后的对象
					...
					$(this).each(function(index,item){//此处循环是因为选择器可能选择了页面上多个轮播
						...
						$(item).find("li>a>img").css({"width":boxwidth,"height":boxheight});//修改图片的宽度和高度
						...
						if(isShowNumber)
						{
						   $(item).find(".controls>li").css({"width":prams.controlsW,"height":prams.controlsH,"line-height":prams.controlsH+"px","border-radius":prams.radius,"padding":"0","display":"block","border":"0"});
						}
						else
						{
						   $(item).find(".controls>li").text("").css({"width":prams.controlsW,"height":prams.controlsH,"line-height":prams.controlsH+"px","border-radius":prams.radius,"padding":"0","display":"block","border":"0"});
						}
